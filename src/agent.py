...
def _start_profile_server(host: str, port: int):
    """
    Start a tiny HTTP server that accepts POST /profile to save a JSON profile to disk,
...
if __name__ == "__main__":
    # On Railway, PORT is provided by the platform. For local dev it defaults to 8080.
    port = int(os.getenv("PORT", "8080"))
    logger.info(f"Starting profile server on 0.0.0.0:{port}")
    _start_profile_server(host="0.0.0.0", port=port)
